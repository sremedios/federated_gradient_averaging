...
def transfer_weights(URL, client_weights, client_headers, sleep_delay=0.01):
    ########## SEND WEIGHTS ##########
    put_successful = False
    while not put_successful:
        data = pickle.dumps(client_weights)
        response = requests.put(
            URL + "put_cyclic_weights",
            data=data, 
    ...
    while server_weights is None:
        ...
        try:
            response = requests.get(URL + "get_cyclic_weights", headers=client_headers)
...
if __name__ == '__main__':
    ...
    while(True):
        ...
        server_weights = transfer_weights(URL, client_weights, client_headers)
